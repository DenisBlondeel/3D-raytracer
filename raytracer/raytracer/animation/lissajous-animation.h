...
{
	template<typename T>
	Animation<T> lissajous(const math::Interval<T>& animation_interval, animation::Duration duration)
	{
	...
	template<typename T>
	Animation<T> lissajous(T from, T to, animation::Duration duration)
	{
		return lissajous(math::interval(from, to), duration);
	}
}